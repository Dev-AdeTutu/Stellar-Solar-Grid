...
const useJson =
  (process.env.LOG_FORMAT ?? (isProduction ? "json" : "text")) === "json";
...
const developmentFormat = winston.format.combine(
  ...
  winston.format.printf(
    ({ timestamp, level, message, request_id, ...meta }) => {
...
function makeLogFn(
  ...
  return (
    msgOrMeta: string | Meta,
    msgOrMeta2?: string | Meta,
    ...rest: unknown[]
  ) => {
    if (typeof msgOrMeta === "string") {
      const meta =
        typeof msgOrMeta2 === "object" && msgOrMeta2 !== null
          ? msgOrMeta2
          : {};
      ...
      winstonLogger.log(
        ...
        msgOrMeta,
        withRequestIdMeta(meta as Meta),
      );
    ...
    winstonLogger.log(
      level === "fatal" ? "error" : level,
      msg,
      withRequestIdMeta(msgOrMeta as Meta),
    );
  };
}
