...
webhookRouter.post("/sms-payment", async (req, res) => {
  ...
  if (!signature || !verifySignature((req as any).rawBody ?? Buffer.from(JSON.stringify(req.body)), signature)) {
  ...
  if (!meter_id || typeof amount_xlm !== "number" || amount_xlm <= 0) {
    return res.status(400).json({ error: "meter_id and positive amount_xlm are required" });
  }
  ...
  try {
    const hash = await adminInvoke("make_payment", [
      ...
      // Default to Daily plan for SMS payments
      StellarSdk.xdr.ScVal.scvVec([
        StellarSdk.xdr.ScVal.scvSymbol("Daily"),
      ]),
    ]);
