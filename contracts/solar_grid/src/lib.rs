...
mod tests {
    ...
    #[test]
    fn test_meter_count_snapshot_after_register_and_deregister() {
        let (env, client, _admin, _token_address) = setup_with_token();
        let owner = Address::generate(&env);
        client.allowlist_add(&owner);
        assert_eq!(client.get_meter_count(), 0);

        let meter1 = String::from_str(&env, "CNT_A");
        let meter2 = String::from_str(&env, "CNT_B");

        client.register_meter(&meter1, &owner);
        client.register_meter(&meter2, &owner);
        assert_eq!(client.get_meter_count(), 2);
        assert_eq!(client.get_meters_by_owner(&owner).len(), 2);
        assert_eq!(client.get_all_meters().len(), 2);

        client.deregister_meter(&meter1);
        assert_eq!(client.get_meter_count(), 1);
        assert!(matches!(
            client.try_get_meter(&meter1),
            Err(Ok(ContractError::MeterNotFound))
        ));
        assert_eq!(client.get_meters_by_owner(&owner).len(), 1);
        assert_eq!(client.get_all_meters().len(), 1);
    }
